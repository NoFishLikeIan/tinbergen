...
In order to study the effect of groups on the equilibria of the game we first look at a simple simulation run with, as before, $N = 5$ and $M = 4$ (Figure \ref{fig:small_avg}). In particular here at $t = 5$, the local market $1$ and $2$ merge. This induces tacit collusion with high quantities, hence low prices. The other two markets settle on a tacit collusion equilibrium with low quantities. At $t = 73$ and t = $88$, $\set{1, 2}$, $3$, and $4$ are merged. This increases competition and brings the market out of collusion.
